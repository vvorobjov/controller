...
#include "tracking_neuron_planner.h"

// uncomment the next line to enable printing of detailed debug information
// #define DEBUG
void
register_tracking_neuron_planner( const std::string& name )
{
  nest::register_node_model< tracking_neuron_planner >( name );
}

// ---------------------------------------------------------------------------
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<tracking_neuron_planner> tracking_neuron_planner::recordablesMap_;
namespace nest
{

  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
template <> void RecordablesMap<tracking_neuron_planner>::create()
  {
    // add state variables to recordables map
   insert_(tracking_neuron_planner_names::_out_rate, &tracking_neuron_planner::get_out_rate);
   insert_(tracking_neuron_planner_names::_lambda, &tracking_neuron_planner::get_lambda);
   insert_(tracking_neuron_planner_names::_curr_traj, &tracking_neuron_planner::get_curr_traj);

    // Add vector variables  
  }
}
std::vector< std::tuple< int, int > > tracking_neuron_planner::rport_to_nestml_buffer_idx =
{
  
  { tracking_neuron_planner::EXC_SPIKES, tracking_neuron_planner::INH_SPIKES },
  { tracking_neuron_planner::SPIKES, tracking_neuron_planner::PORT_NOT_AVAILABLE },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of tracking_neuron_planner's constructor.
// ---------------------------------------------------------------------------

tracking_neuron_planner::Parameters_::Parameters_()
{
}

tracking_neuron_planner::State_::State_()
{
}
...
tracking_neuron_planner::Buffers_::Buffers_(tracking_neuron_planner &n):
  logger_(n)
...
tracking_neuron_planner::Buffers_::Buffers_(const Buffers_ &, tracking_neuron_planner &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
  , spike_input_received_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
...
tracking_neuron_planner::tracking_neuron_planner():ArchivingNode(), P_(), S_(), B_(*this)
{
...
tracking_neuron_planner::tracking_neuron_planner(const tracking_neuron_planner& __n):
...
tracking_neuron_planner::~tracking_neuron_planner()
{
}
...
// ---------------------------------------------------------------------------
void tracking_neuron_planner::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void tracking_neuron_planner::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] tracking_neuron_planner::init_state_internal_()" << std::endl;
#endif
...
void tracking_neuron_planner::init_buffers_()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] tracking_neuron_planner::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void tracking_neuron_planner::recompute_internal_variables(bool exclude_timestep)
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
void tracking_neuron_planner::pre_run_hook()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] tracking_neuron_planner::pre_run_hook()" << std::endl;
#endif
...
void tracking_neuron_planner::update(nest::Time const & origin, const long from, const long to)
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
#ifdef DEBUG
    std::cout << "[neuron " << this << "] tracking_neuron_planner::update: handling post spike at t = " << get_t() << std::endl;
#endif
    /**
...
void tracking_neuron_planner::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}


void tracking_neuron_planner::handle(nest::SpikeEvent &e)
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] tracking_neuron_planner::handle(SpikeEvent)" << std::endl;
#endif
  ...
  else
  {
    ...
    if ( nestml_buffer_idx == tracking_neuron_planner::PORT_NOT_AVAILABLE )
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
