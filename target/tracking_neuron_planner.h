 ...
 *
 *  Generated from NESTML 8.2.0 at time: 2025-11-07 10:59:43.356091
**/
#ifndef TRACKING_NEURON_PLANNER
#define TRACKING_NEURON_PLANNER
...
namespace nest
{
namespace tracking_neuron_planner_names
{
...
void register_tracking_neuron_planner( const std::string& name );

class tracking_neuron_planner : public nest::ArchivingNode
{
public:
  /**
   * The constructor is only used to create the model prototype in the model manager.
  **/
  tracking_neuron_planner();
  ...
  **/
  tracking_neuron_planner(const tracking_neuron_planner &);

  /**
   * Destructor.
  **/
  ~tracking_neuron_planner() override;
...
static std::vector< std::tuple< int, int > > rport_to_nestml_buffer_idx;
  ...
  **/
  ...
  friend class nest::RecordablesMap<tracking_neuron_planner>;
  friend class nest::UniversalDataLogger<tracking_neuron_planner>;
  ...
  **/
  struct Buffers_
  {
    Buffers_(tracking_neuron_planner &);
    Buffers_(const Buffers_ &, tracking_neuron_planner &);

    /**
     * Logger for all analog data
    **/
    nest::UniversalDataLogger<tracking_neuron_planner> logger_;
  ...
  **/
  ...
  static nest::RecordablesMap<tracking_neuron_planner> recordablesMap_;
...
}; /* neuron tracking_neuron_planner */

inline nest_port_t tracking_neuron_planner::send_test_event(nest::Node& target, nest_rport_t receptor_type, nest::synindex, bool)
{
...
inline nest_port_t tracking_neuron_planner::handles_test_event(nest::SpikeEvent&, nest_port_t receptor_type)
{
    // You should usually not change the code in this function.
...
inline nest_port_t tracking_neuron_planner::handles_test_event(nest::DataLoggingRequest& dlr, nest_port_t receptor_type)
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline void tracking_neuron_planner::get_status(DictionaryDatum &__d) const
{
  // parameters
  def< double >(__d, nest::tracking_neuron_planner_names::_kp, get_kp());
  def< bool >(__d, nest::tracking_neuron_planner_names::_pos, get_pos());
  def< double >(__d, nest::tracking_neuron_planner_names::_base_rate, get_base_rate());
  def< long >(__d, nest::tracking_neuron_planner_names::_simulation_steps, get_simulation_steps());
  def< std::vector< double >  >(__d, nest::tracking_neuron_planner_names::_traj, get_traj());

  // initial values for state variables in ODE or kernel
  def< double >(__d, nest::tracking_neuron_planner_names::_out_rate, get_out_rate());
  def< double >(__d, nest::tracking_neuron_planner_names::_lambda, get_lambda());
  def< long >(__d, nest::tracking_neuron_planner_names::_spike_count_out, get_spike_count_out());
  def< long >(__d, nest::tracking_neuron_planner_names::_current_step, get_current_step());
  def< double >(__d, nest::tracking_neuron_planner_names::_curr_traj, get_curr_traj());
...
inline void tracking_neuron_planner::set_status(const DictionaryDatum &__d)
{
  // parameters
  double tmp_kp = get_kp();
  nest::updateValueParam<double>(__d, nest::tracking_neuron_planner_names::_kp, tmp_kp, this);
  // Resize vectors
  if (tmp_kp != get_kp())
  {
  }
  bool tmp_pos = get_pos();
  nest::updateValueParam<bool>(__d, nest::tracking_neuron_planner_names::_pos, tmp_pos, this);
  // Resize vectors
  if (tmp_pos != get_pos())
  {
  }
  double tmp_base_rate = get_base_rate();
  nest::updateValueParam<double>(__d, nest::tracking_neuron_planner_names::_base_rate, tmp_base_rate, this);
  // Resize vectors
  if (tmp_base_rate != get_base_rate())
  {
  }
  long tmp_simulation_steps = get_simulation_steps();
  nest::updateValueParam<long>(__d, nest::tracking_neuron_planner_names::_simulation_steps, tmp_simulation_steps, this);
  // Resize vectors
  ...
  updateValue<std::vector< double > >(__d, nest::tracking_neuron_planner_names::_traj, tmp_traj);
  // Resize vectors
  ...
  nest::updateValueParam<double>(__d, nest::tracking_neuron_planner_names::_out_rate, tmp_out_rate, this);
  double tmp_lambda = get_lambda();
  nest::updateValueParam<double>(__d, nest::tracking_neuron_planner_names::_lambda, tmp_lambda, this);
  long tmp_spike_count_out = get_spike_count_out();
  nest::updateValueParam<long>(__d, nest::tracking_neuron_planner_names::_spike_count_out, tmp_spike_count_out, this);
  long tmp_current_step = get_current_step();
  nest::updateValueParam<long>(__d, nest::tracking_neuron_planner_names::_current_step, tmp_current_step, this);
  double tmp_curr_traj = get_curr_traj();
  nest::updateValueParam<double>(__d, nest::tracking_neuron_planner_names::_curr_traj, tmp_curr_traj, this);
...
#endif /* #ifndef TRACKING_NEURON_PLANNER */
