...
// #define DEBUG
void
register_rb_neuron( const std::string& name )
{
  nest::register_node_model< rb_neuron >( name );
}
...
template <> void DynamicRecordablesMap<rb_neuron>::create(rb_neuron& host)
  {
    insert("in_rate", host.get_data_access_functor( rb_neuron::State_::IN_RATE ));
    insert("out_rate", host.get_data_access_functor( rb_neuron::State_::OUT_RATE ));
    insert("spike_count_in", host.get_data_access_functor( rb_neuron::State_::SPIKE_COUNT_IN ));
    insert("lambda_poisson", host.get_data_access_functor( rb_neuron::State_::LAMBDA_POISSON ));

    // Add vector variables  
      host.insert_recordables();
  }
}
  std::string rb_neuron::get_var_name(size_t elem, std::string var_name)
  {
    std::stringstream n;
    n << var_name << elem;
    return n.str();
  ...
  void rb_neuron::insert_recordables(size_t first)
  {
      for (size_t i = 0; i < 
P_.simulation_steps; i++)
      {
        size_t elem = rb_neuron::State_::SPIKES_BUFFER + i;
        recordablesMap_.insert(get_var_name(i, "SPIKES_BUFFER_"), this->get_data_access_functor(elem));
      }
  }

  nest::DataAccessFunctor< rb_neuron >
  rb_neuron::get_data_access_functor( size_t elem )
  {
    return nest::DataAccessFunctor< rb_neuron >( *this, elem );
  }

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of rb_neuron's constructor.
// ---------------------------------------------------------------------------
...
rb_neuron::rb_neuron():ArchivingNode(), P_(), S_(), B_(*this)
{
...
// ---------------------------------------------------------------------------
void rb_neuron::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
    "rb_neuron",
...
void rb_neuron::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] rb_neuron::init_state_internal_()" << std::endl;
#endif
...
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] rb_neuron::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void rb_neuron::recompute_internal_variables(bool exclude_timestep)
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
void rb_neuron::pre_run_hook()
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] rb_neuron::pre_run_hook()" << std::endl;
#endif
...
#ifdef DEBUG
    std::cout << "[neuron " << this << "] rb_neuron::update: handling post spike at t = " << get_t() << std::endl;
#endif
    /**
...
{
#ifdef DEBUG
  std::cout << "[neuron " << this << "] rb_neuron::handle(SpikeEvent)" << std::endl;
#endif
