 ...
 *
 *  Generated from NESTML 8.2.0 at time: 2025-11-07 10:59:41.865182
**/
#ifndef RB_NEURON
#define RB_NEURON
...
void register_rb_neuron( const std::string& name );

class rb_neuron : public nest::ArchivingNode
{
public:
  /**
...
private:
  ...
  **/
  ...
  friend class nest::DynamicRecordablesMap< rb_neuron >;
  friend class nest::DynamicUniversalDataLogger< rb_neuron >;
  friend class nest::DataAccessFunctor< rb_neuron >;
...
enum StateVecVars {
  ...
  **/
  struct Buffers_
  {
    ...
    **/
    nest::DynamicUniversalDataLogger<rb_neuron> logger_;
  ...
  **/
  ...
  nest::DynamicRecordablesMap<rb_neuron> recordablesMap_;
  nest::DataAccessFunctor< rb_neuron > get_data_access_functor( size_t elem );
...
inline nest_port_t rb_neuron::send_test_event(nest::Node& target, nest_rport_t receptor_type, nest::synindex, bool)
{
...
inline nest_port_t rb_neuron::handles_test_event(nest::SpikeEvent&, nest_port_t receptor_type)
{
    // You should usually not change the code in this function.
...
inline nest_port_t rb_neuron::handles_test_event(nest::DataLoggingRequest& dlr, nest_port_t receptor_type)
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
{
  // parameters
  def< double >(__d, nest::rb_neuron_names::_kp, get_kp());
  def< double >(__d, nest::rb_neuron_names::_base_rate, get_base_rate());
  def< double >(__d, nest::rb_neuron_names::_max_peak_rate, get_max_peak_rate());
  def< double >(__d, nest::rb_neuron_names::_buffer_size, get_buffer_size());
  def< long >(__d, nest::rb_neuron_names::_simulation_steps, get_simulation_steps());
  def< double >(__d, nest::rb_neuron_names::_sdev, get_sdev());
  def< double >(__d, nest::rb_neuron_names::_desired, get_desired());

  // initial values for state variables in ODE or kernel
  def< double >(__d, nest::rb_neuron_names::_in_rate, get_in_rate());
  def< double >(__d, nest::rb_neuron_names::_out_rate, get_out_rate());
  def< double >(__d, nest::rb_neuron_names::_spike_count_in, get_spike_count_in());
  def< long >(__d, nest::rb_neuron_names::_spike_count_out, get_spike_count_out());
  def< long >(__d, nest::rb_neuron_names::_tick, get_tick());
  def< double >(__d, nest::rb_neuron_names::_lambda_poisson, get_lambda_poisson());
  def< std::vector< double >  >(__d, nest::rb_neuron_names::_spikes_buffer, get_spikes_buffer());
...
inline void rb_neuron::set_status(const DictionaryDatum &__d)
{
  ...
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_kp, tmp_kp, this);
  // Resize vectors
  if (tmp_kp != get_kp())
  {
  }
  double tmp_base_rate = get_base_rate();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_base_rate, tmp_base_rate, this);
  // Resize vectors
  if (tmp_base_rate != get_base_rate())
  {
  }
  double tmp_max_peak_rate = get_max_peak_rate();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_max_peak_rate, tmp_max_peak_rate, this);
  // Resize vectors
  if (tmp_max_peak_rate != get_max_peak_rate())
  {
  }
  double tmp_buffer_size = get_buffer_size();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_buffer_size, tmp_buffer_size, this);
  // Resize vectors
  if (tmp_buffer_size != get_buffer_size())
  {
  }
  long tmp_simulation_steps = get_simulation_steps();
  nest::updateValueParam<long>(__d, nest::rb_neuron_names::_simulation_steps, tmp_simulation_steps, this);
  // Resize vectors
  ...
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_sdev, tmp_sdev, this);
  // Resize vectors
  if (tmp_sdev != get_sdev())
  {
  }
  double tmp_desired = get_desired();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_desired, tmp_desired, this);
  // Resize vectors
  ...
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_in_rate, tmp_in_rate, this);
  double tmp_out_rate = get_out_rate();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_out_rate, tmp_out_rate, this);
  double tmp_spike_count_in = get_spike_count_in();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_spike_count_in, tmp_spike_count_in, this);
  long tmp_spike_count_out = get_spike_count_out();
  nest::updateValueParam<long>(__d, nest::rb_neuron_names::_spike_count_out, tmp_spike_count_out, this);
  long tmp_tick = get_tick();
  nest::updateValueParam<long>(__d, nest::rb_neuron_names::_tick, tmp_tick, this);
  double tmp_lambda_poisson = get_lambda_poisson();
  nest::updateValueParam<double>(__d, nest::rb_neuron_names::_lambda_poisson, tmp_lambda_poisson, this);
  std::vector< double >  tmp_spikes_buffer = get_spikes_buffer();
  updateValue<std::vector< double > >(__d, nest::rb_neuron_names::_spikes_buffer, tmp_spikes_buffer);
